...
@unittest.skipIf(
    platform.system() == "Windows", "Concurrent isn't supported on windows"
)
class TestIntegration(BaseTestCase):
    ...
    def test_str_repr(self):
        ...
        self.assertEqual(
            repr(cab), "ConcurrentMerge({0}, ({1!r}, {2!r}))".format(operator.add, a, b)
        )
    ...
    def test_no_fork(self, mock_wait, mock_join, mock_close, mock_exit, mock_fork):
        ...
        for test_name in (
            t for t in dir(self) if t.startswith("test_") and t != this_test
        ):
